...
namespace Rf.Lotto.Combinatorics
{
  /// <summary>
  ...
  public class Variations<T> : IMetaCollection<T>
  {
    ...
    public Variations(IEnumerable<T> values, int sampleSize)
    {
      Initialize(values.ToList(), sampleSize, GenerateOption.WithoutRepetition);
    }
